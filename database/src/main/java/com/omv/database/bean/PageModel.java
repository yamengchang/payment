...
public class PageModel<T> implements Serializable {
    ...
    public static void addUserIdParam(Map<String, Object> params) {
        if (!UserUtils.isAdmin()) {
            params.put("userId", UserUtils.getUserId());
        }
    }

    public static PageModel createPageMould(Map<String, Object> params, Integer pageNo, Integer pageSize) {
        MapUtil.cleanNull(params);
        PageModel pageModel = new PageModel(pageNo == null ? 1 : pageNo, pageSize == null ? 10 : pageSize);
        if (null != params.get("showFields")) {
            pageModel.setFields(params.remove("showFields").toString());
        }
        if (ObjectUtils.allNotNull(params.get("pageNo")))
            params.remove(params.remove("pageNo").toString());
        if (ObjectUtils.allNotNull(params.get("pageSize")))
        ...
        param.forEach((ka, v) -> {
            ...
            switch (type) {
                ...
                case "gt":
                    ...
                    break;
                case "lte":
                    addCondition(ConditionType.lessThanEquals, k, v, clzPath);
                    break;
                case "gte":
                    addCondition(ConditionType.greaterThanEquals, k, v, clzPath);
                    break;
                case "lt":
                ...
                case "in":
                    ...
                    break;
                case "asc":
                    addCondition(ConditionType.asc, k, v, clzPath);
                    break;
                case "desc":
                    addCondition(ConditionType.desc, k, v, clzPath);
                    break;
                case "nin":
                    addCondition(ConditionType.notIn, k, v, clzPath);
                    break;
                default:
                    try {
            ...
            Specification specification = new Specification<T>() {
                ...
                public Predicate toPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
                    ...
                    for (int i = 0; i < conditions.size(); i++) {
                        ...
                        if (queryCondition.getConditionType().equals(ConditionType.asc) ||
                                queryCondition.getConditionType().equals(ConditionType.desc)) {
                            queryCondition.sortBy(root, query, cb);
                        } else {
                            predicate.getExpressions().add(queryCondition.getExpression(root, cb));
                        }
                    }
                    return predicate;
                }
