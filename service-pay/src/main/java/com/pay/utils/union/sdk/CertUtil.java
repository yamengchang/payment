 ...
 * =============================================================================
 */
package com.pay.utils.union.sdk;

import com.omv.common.util.basic.ByteUtils;
import com.omv.common.util.file.FileUtils;
...
import static com.pay.utils.union.sdk.SDKConstants.UNIONPAY_CNNAME;
import static com.pay.utils.union.sdk.SDKUtil.isEmpty;
...
public class CertUtil {
     ...
     * @return
     */
    public static PrivateKey getSignCertPrivateKey(String base64File, String certPwd) {
        try {
            KeyStore keyStore = null;
            try {
                keyStore = getKeyInfoByBase64File(base64File,
                        certPwd, SDKConfig.getConfig().getSignCertType());
            } catch (IOException e) {
                e.printStackTrace();
            }
...
//			cert.verify(middleCert.getPublicKey());
     ...
     * @return
     */
    public static boolean verifyCertificateByMiddleAndRootCert(X509Certificate cert) {
...
//			cert.verify(middleCert.getPublicKey());
    ...
    public static String getSignCertId(String base64File, String certPwd) throws IOException, KeyStoreException {
        KeyStore keyStore = getKeyInfoByBase64File(base64File,
                certPwd, SDKConfig.getConfig().getSignCertType());
...
//				+ getSignCertId() + "]");
    ...
    public static KeyStore getKeyInfoByBase64File(String base64File, String certPwd, String signCertType) throws IOException {
        InputStream input = null;
        try {
            KeyStore ks = KeyStore.getInstance(SDKConfig
                    .getConfig().getSignCertType(), "BC");
            LogUtil.writeLog("Load RSA CertFile");
            byte[] fileByte = com.omv.common.util.signature.Base64.decode(base64File);
            input = new ByteArrayInputStream(fileByte);
            char[] nPassword = null;
            nPassword = null == certPwd || "".equals(certPwd.trim()) ? null : certPwd.toCharArray();
            if (null != ks) {
    ...
    public static boolean verifyCertificateWithPubKeyFile(X509Certificate cert, String middleCertStr, String rootCertStr) {
        if (null == cert) {
            LogUtil.writeErrorLog("cert must Not null");
            return false;
        }
        try {
            ...
            if (!verifyCertificateChainByMiddleAndRootCert(cert, middleCertStr, rootCertStr)) {
                return false;
            }
...
//                LogUtil.writeErrorLog("cer owner is not CUP:" + CertUtil.getIdentitiesFromCertficate(cert));
    ...
    private static boolean verifyCertificateChainByMiddleAndRootCert(X509Certificate cert, String middleCertStr, String rootCertStr) {
        X509Certificate middleCert = getMiddleCertWithPubKeyFile(middleCertStr);

        X509Certificate rootCert = getRootCertWithPubKeyFile(rootCertStr);
    ...
    public static X509Certificate getMiddleCertWithPubKeyFile(String pubKeyFile) {
        X509Certificate encryptCertTemp = null;
        CertificateFactory cf = null;
        InputStream in = null;
        try {
            cf = CertificateFactory.getInstance("X.509", "BC");
            in = new ByteArrayInputStream(Base64.decode(pubKeyFile));
            encryptCertTemp = (X509Certificate) cf.generateCertificate(in);
            // 打印证书加载信息,供测试阶段调试
        } catch (CertificateException e) {
            LogUtil.writeErrorLog("InitCert Error", e);
        ...
        if (null == encryptCertTemp) {
            LogUtil.writeErrorLog("middleCert must Not null");
        }
        return encryptCertTemp;
    }

    public static X509Certificate getRootCertWithPubKeyFile(String rootCertStr) {
        X509Certificate encryptCertTemp = null;
        CertificateFactory cf = null;
        ByteArrayInputStream in = null;
        try {
            cf = CertificateFactory.getInstance("X.509", "BC");
            in = new ByteArrayInputStream(Base64.decode(rootCertStr));
            encryptCertTemp = (X509Certificate) cf.generateCertificate(in);
            // 打印证书加载信息,供测试阶段调试
        } catch (CertificateException e) {
            LogUtil.writeErrorLog("InitCert Error", e);
